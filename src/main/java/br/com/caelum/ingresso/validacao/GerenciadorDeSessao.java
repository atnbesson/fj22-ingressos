...
public class GerenciadorDeSessao {
	...
	public boolean cabe(Sessao sessaoNova) {
		...
		return sessoesDaSala.stream().noneMatch(seesaoExistente -> 
		horarioIsConflitante(seesaoExistente, sessaoNova));
	...
	public boolean horarioIsConflitante(Sessao sessaoExistente, Sessao sessaoNova) {
		...
		boolean sessaoNovaComecaDespoisDaExistente = terminoSessaoExistente.isBefore(inicioSessaoNova);
		
		if(sessaoNovaTerminaAntesDaExistente || sessaoNovaComecaDespoisDaExistente) {
			return false;
		}
		return true;
	}
